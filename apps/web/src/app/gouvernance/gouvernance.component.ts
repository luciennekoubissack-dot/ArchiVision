...
import { Solution, SolutionService } from '../opportunites/solution.service';
  ...
  template: `
    ...
    <section *ngIf="tab === 'politiques'">
      <div class="page-header">
        <h3>Politiques ({{ politiquesTotal }})</h3>
      ...
      <section class="card">
        ...
        <app-pagination [page]="politiquesPage" [total]="politiquesTotal" [pageSize]="politiquesPageSize" (pageChange)="onPolitiquesPageChange($event)" />
      </section>
    </section>
    ...
    <section *ngIf="tab === 'conformite'">
      <div class="empty-state card" *ngIf="solutions.length === 0 || politiquesAll.length === 0">
        La matrice de conformité nécessite au moins une solution (module Opportunités & solutions) et une politique.
      </div>
      <section class="card" *ngIf="solutions.length > 0 && politiquesAll.length > 0">
        ...
        <div class="table-scroll">
          <table class="table">
            <thead>
              <tr>
                <th>Solution</th>
                <th *ngFor="let p of politiquesAll">{{ p.nom }}</th>
                <th></th>
              </tr>
            </thead>
            <tbody>
              <tr *ngFor="let s of solutions">
                <td>{{ s.nom }}</td>
                <td *ngFor="let p of politiquesAll">
    ...
    <section *ngIf="tab === 'changements'">
      <div class="page-header">
        <h3>Demandes de changement ({{ changementsTotal }})</h3>
      ...
      <section class="card">
        ...
        <app-pagination [page]="changementsPage" [total]="changementsTotal" [pageSize]="changementsPageSize" (pageChange)="onChangementsPageChange($event)" />
      </section>
    </section>
...
export class GouvernanceComponent implements OnInit {
  ...
  politiquesPage = 1;
  politiquesTotal = 0;
  politiquesPageSize = DEFAULT_PAGE_SIZE;
  /** Toutes les politiques, sans pagination : en-têtes de colonnes de la matrice de conformité. */
  politiquesAll: Politique[] = [];
  ...
  loadPolitiques(): void {
    this.politiqueService.listPaginated(this.politiquesPage, this.politiquesPageSize).subscribe({
      next: (result) => {
        this.politiques = result.items;
        this.politiquesTotal = result.total;
      },
      error: () => this.toast.error('Impossible de charger les politiques.'),
    });
  }

  loadPolitiquesAll(): void {
    this.politiqueService.list().subscribe({
      next: (politiques) => (this.politiquesAll = politiques),
      error: () => this.toast.error('Impossible de charger les politiques.'),
    });
  }

  onPolitiquesPageChange(page: number): void {
    this.politiquesPage = page;
    this.loadPolitiques();
  }
  ...
  loadChangements(): void {
    this.changementService.listPaginated(this.changementsPage, this.changementsPageSize).subscribe({
      next: (result) => {
        this.changements = result.items;
        this.changementsTotal = result.total;
      },
  ...
  onChangementsPageChange(page: number): void {
  ...
  saveRow(s: Solution): void {
    ...
    const items: ConformiteItem[] = this.politiquesAll
