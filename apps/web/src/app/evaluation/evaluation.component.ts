  ...
  template: `
    ...
    <section *ngIf="tab === 'reponses'">
      ...
      <section class="card">
        ...
        <app-pagination [page]="reponsesPage" [total]="reponsesTotal" [pageSize]="reponsesPageSize" (pageChange)="onReponsesPageChange($event)" />
      </section>
    </section>
    ...
    <section *ngIf="tab === 'rapport'">
      <div class="stats-grid">
        ...
        <section class="card stat">
          <span class="stat-value">{{ reponsesAll.length }}</span>
          <span class="stat-label">Réponses collectées</span>
        </section>
      </div>

      <section class="card" *ngIf="reponsesAll.length > 0">
...
export class EvaluationComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  reponsesPage = 1;
  reponsesTotal = 0;
  reponsesPageSize = DEFAULT_PAGE_SIZE;
  /** Toutes les réponses, sans pagination : note moyenne, graphique et commentaires du rapport. */
  reponsesAll: EnqueteReponse[] = [];
  ...
  get noteMoyenne(): number | null {
    if (this.reponsesAll.length === 0) return null;
    const somme = this.reponsesAll.reduce((acc, r) => acc + r.score, 0);
    return Math.round((somme / this.reponsesAll.length) * 10) / 10;
  }

  get commentaires(): EnqueteReponse[] {
    return this.reponsesAll.filter((r) => r.commentaire);
  }
  ...
  private loadPage(): void {
    this.enqueteService.listPaginated(this.reponsesPage, this.reponsesPageSize).subscribe({
      next: (result) => {
        this.reponses = result.items;
        this.reponsesTotal = result.total;
      },
  ...
  loadAll(): void {
    this.enqueteService.list().subscribe({
      next: (reponses) => (this.reponsesAll = reponses),
      error: () => this.toast.error('Impossible de charger les réponses.'),
    });
  }

  onReponsesPageChange(page: number): void {
    this.reponsesPage = page;
    this.loadPage();
  }
  ...
  private renderChart(): void {
    if (!this.viewReady || !this.scoresChartRef || this.reponsesAll.length === 0) return;
    this.chart?.destroy();

    const counts = [1, 2, 3, 4, 5].map((score) => this.reponsesAll.filter((r) => r.score === score).length);
