  ...
  template: `
      ...
      <section class="card identity-card">
        <div class="identity-summary">
          ...
          <div class="identity-text">
            ...
            <span class="muted">{{ partiesPrenantesTotal }} renseignée(s)</span>
          </div>
          <div class="identity-actions">
    ...
    <div class="popover-backdrop" *ngIf="partiesMode === 'view'" (click)="partiesMode = null">
      <div class="popover-card" (click)="$event.stopPropagation()">
        ...
        <div class="empty-state" *ngIf="partiesPrenantesTotal === 0">Aucune partie prenante renseignée.</div>
        <div class="table-scroll" *ngIf="partiesPrenantesTotal > 0">
          <table class="table">
        ...
        <app-pagination
          [page]="partiesPrenantesPage"
          [total]="partiesPrenantesTotal"
          [pageSize]="partiesPrenantesPageSize"
          (pageChange)="onPartiesPrenantesPageChange($event)"
        />
      </div>
    </div>
    ...
    <section *ngIf="tab === 'membres' && canManageMembres">
      ...
      <section class="card">
        ...
        <app-pagination [page]="membresPage" [total]="membresTotal" [pageSize]="membresPageSize" (pageChange)="onMembresPageChange($event)" />
      </section>
    </section>
...
export class OrganisationComponent implements OnInit {
  ...
  membresPage = 1;
  membresTotal = 0;
  membresPageSize = DEFAULT_PAGE_SIZE;
  ...
  partiesPrenantesPage = 1;
  partiesPrenantesTotal = 0;
  partiesPrenantesPageSize = DEFAULT_PAGE_SIZE;
  ...
  onMembresPageChange(page: number): void {
    this.membresPage = page;
    this.loadMembres();
  }

  loadMembres(): void {
    this.membresService.listPaginated(this.membresPage, this.membresPageSize).subscribe({
      next: (result) => {
        this.membres = result.items;
        this.membresTotal = result.total;
      },
  ...
  loadPartiesPrenantes(): void {
    this.partiesPrenantesService.listPaginated(this.partiesPrenantesPage, this.partiesPrenantesPageSize).subscribe({
      next: (result) => {
        this.partiesPrenantes = result.items;
        this.partiesPrenantesTotal = result.total;
      },
      error: () => this.toast.error('Impossible de charger les parties prenantes.'),
    });
  }

  onPartiesPrenantesPageChange(page: number): void {
    this.partiesPrenantesPage = page;
    this.loadPartiesPrenantes();
  }
    ...
    this.partiesPrenantesService.list().subscribe({
      next: (parties) =>
        exportToExcel(
          'parties-prenantes',
          'Parties prenantes',
          parties.map((p) => ({ Nom: p.nom, Rôle: p.role ?? '' })),
        ),
      error: () => this.toast.error("Impossible d'exporter les parties prenantes."),
    });
  }
