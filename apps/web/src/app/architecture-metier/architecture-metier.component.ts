...
@Component({
  ...
  template: `
    ...
    <section *ngIf="mainTab === 'archimate' && tab === 'capacites'">
      <div class="page-header">
        <h3>Capacités ({{ capacitesTotal }})</h3>
      ...
      <section class="card">
        ...
        <app-pagination [page]="capacitesPage" [total]="capacitesTotal" [pageSize]="capacitesPageSize" (pageChange)="onCapacitesPageChange($event)" />
      </section>
    </section>
    ...
    <section *ngIf="mainTab === 'archimate' && tab === 'elements'">
      ...
      <section class="card">
        ...
        <app-pagination [page]="elementsPage" [total]="elementsTotal" [pageSize]="elementsPageSize" (pageChange)="onElementsPageChange($event)" />
      </section>
    </section>
      ...
      <form class="popover-card" (click)="$event.stopPropagation()" (submit)="createElement($event)">
        ...
        <label class="field">
          ...
          <select [value]="newElement.capaciteMetierId || ''" (change)="newElement.capaciteMetierId = $any($event.target).value || undefined">
            ...
            <option *ngFor="let c of capacitesAll" [value]="c.id">{{ c.nom }}</option>
          </select>
        </label>
        ...
        <div class="grid-2">
          <label class="field">
            ...
            <select [value]="newRelation.sourceId" (change)="newRelation.sourceId = $any($event.target).value">
              ...
              <option *ngFor="let e of elementsAll" [value]="e.id">{{ e.nom }}</option>
            </select>
          </label>
          <label class="field">
            Cible
...
export class ArchitectureMetierComponent implements OnInit {
  ...
  capacitesPage = 1;
  capacitesTotal = 0;
  capacitesPageSize = DEFAULT_PAGE_SIZE;
  /** Toutes les capacités, sans pagination : source du menu déroulant dans les formulaires Éléments. */
  capacitesAll: CapaciteMetier[] = [];
  ...
  elementsPage = 1;
  elementsTotal = 0;
  elementsPageSize = DEFAULT_PAGE_SIZE;
  /** Tous les éléments (tous types confondus), sans pagination : source des menus déroulants Source/Cible du formulaire Relations. */
  elementsAll: ElementArchimate[] = [];
  ...
  loadCapacites(): void {
    this.archimateService.listCapacitesPaginated(this.capacitesPage, this.capacitesPageSize).subscribe({
      next: (result) => {
        this.capacites = result.items;
        this.capacitesTotal = result.total;
      },
      error: () => this.toast.error('Impossible de charger les capacités.'),
    });
  }

  loadCapacitesAll(): void {
    this.archimateService.listCapacites().subscribe({
      next: (capacites) => (this.capacitesAll = capacites),
      error: () => this.toast.error('Impossible de charger les capacités.'),
    });
  }

  onCapacitesPageChange(page: number): void {
    this.capacitesPage = page;
    this.loadCapacites();
  }
  ...
  loadElements(): void {
    this.archimateService.listElementsPaginated(this.elementsPage, this.elementsPageSize, this.typeFilter || undefined).subscribe({
      next: (result) => {
        this.elements = result.items;
        this.elementsTotal = result.total;
      },
      error: () => this.toast.error('Impossible de charger les éléments.'),
    });
  }

  loadElementsAll(): void {
    this.archimateService.listElements().subscribe({
      next: (elements) => (this.elementsAll = elements),
      error: () => this.toast.error('Impossible de charger les éléments.'),
    });
  }

  onElementsPageChange(page: number): void {
  ...
  loadRelations(): void {
    this.archimateService.listRelationsPaginated(this.relationsPage, this.relationsPageSize).subscribe({
      next: (result) => {
